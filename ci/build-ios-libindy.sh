...
if [ "$BUILD_FRAMEWORK" == "true" ]; then
    ...
    if [ ! -d "${FRAMEWORK_DEVICE_PATH}" ]; then
        ...
        xcodebuild -workspace Indy.xcworkspace -scheme Indy -configuration Release -arch arm64 -arch armv7 only_active_arch=no defines_module=yes -sdk "iphoneos" -derivedDataPath "build/iphoneos" clean build BUILD_LIBRARY_FOR_DISTRIBUTION=YES > build/iphoneos/build.log
    else
    ...
    if [ ! -d "${FRAMEWORK_SIMULATOR_PATH}" ]; then
        ...
        xcodebuild -workspace Indy.xcworkspace -scheme Indy -configuration Release -arch x86_64 -arch i386 only_active_arch=no defines_module=yes -sdk "iphonesimulator" -derivedDataPath "build/iphonesimulator" clean build BUILD_LIBRARY_FOR_DISTRIBUTION=YES > build/iphonesimulator/build.log
    else
